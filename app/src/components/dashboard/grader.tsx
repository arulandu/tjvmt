...
const CSVSubmitGrade = ({ tsts }) => {
  ...
  return (
    <div className='mt-2 w-full bg-navy-light bg-opacity-50 p-4'>
      ...
      <Dropdown id="tstId" label="TST" options={tsts.filter(tst => tst.name.includes('2024')).map(tst => ({ label: tst.name, value: tst.id }))} value={input.tstId} onChange={(e) => handleInputChange(e, input, setInput)} className='mt-2' />
