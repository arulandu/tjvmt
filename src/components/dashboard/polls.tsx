...
const CreatePoll = () => {
  const getInput = (block) => {
    ...
    return {"text": `${month} ${d.getDate()}`, "choices": (block == "T" ? "ARML Practice" : `${block} Block`), "password": (block == "T" ? "" : pass)};
  }
  ...
  return (
    <div className='mt-6 sm:w-60 md:w-96 bg-navy-light bg-opacity-50 p-4 mx-auto'>
      ...
      <OutlineButton name='ARML Practice' className='mt-4' onClick={() => setInput(getInput("T"))} />
